// api/video-analyze.js
// Deep video intelligence analysis for Rezai Vision Agency OS
// Supports: YouTube (native), Instagram/TikTok/FB (text+context analysis)
// Returns: structured psychological deconstruction + rebuild brief
...
export default async function handler(req, res) {
  ...
  const {
    mode,          // 'analyze' | 'rebuild' | 'format-finder' | 'hook-generator'
    videoUrl,      // optional: YouTube URL (native Gemini support)
    text,          // caption / hook / script text
    niche,         // e.g. "Recruiting für Pflegeheime"
    platform,      // 'instagram' | 'tiktok' | 'facebook' | 'youtube'
    clientContext, // for rebuild: client name, industry, goal
    analysisResult, // for rebuild: pass previous analysis JSON
  } = req.body;

  const isYouTube = videoUrl && (videoUrl.includes('youtube.com') || videoUrl.includes('youtu.be'));

  // ══════════════════════════════════════════════════════
  // ANALYZE MODE: Deep psychological deconstruction
  // ══════════════════════════════════════════════════════
  if (mode === 'analyze') {
    let parts = [];

    // If YouTube, pass video directly to Gemini
    if (isYouTube) {
      parts.push({
...
Du sezerst Videos wie ein Chirurg: du verstehst was in der ERSTEN SEKUNDE passiert, welche EMOTION ausgelöst wird, warum jemand NICHT wegscrollt, wie der SOUND die Psyche manipuliert und warum der CTA konvertiert.
...
Antworte NUR mit dem JSON-Objekt, ohne Markdown, ohne Codeblöcke, ohne zusätzlichen Text.
    ...
    try {
      const model = isYouTube ? 'gemini-1.5-pro' : 'gemini-2.0-flash';
      ...
      // Clean JSON from any accidental markdown
      const clean = raw.replace(/^```json\n?/m, '').replace(/^```\n?/m, '').replace(/\n?```$/m, '').trim();

      try {
        const parsed = JSON.parse(clean);
        return res.status(200).json({ success: true, analysis: parsed, model, source: isYouTube ? 'video' : 'text' });
      } catch {
        return res.status(200).json({ success: true, analysis: null, rawText: clean, model });
      }
  ...
  // ══════════════════════════════════════════════════════
  // REBUILD MODE: Adapt successful format for client
  // ══════════════════════════════════════════════════════
  if (mode === 'rebuild') {
...

{
  ...
  "sound_empfehlung": {
    ...
    "sound_effekte": ["Effekt 1 bei Sekunde X", "..."]
  },
  ...
  "higgsfield_prompts": [
    "Fertiger Higgsfield KI-Video Prompt 1 (auf Englisch)",
    "Fertiger Higgsfield KI-Video Prompt 2 (auf Englisch)"
  ],
...
Antworte NUR mit dem JSON, ohne Markdown, ohne Codeblöcke.
  ...
  // ══════════════════════════════════════════════════════
  // FORMAT-FINDER: Best performing formats for a niche
  // ══════════════════════════════════════════════════════
...
Antworte NUR mit dem JSON, ohne Markdown.
  ...
  return res.status(400).json({ error: `Unknown mode: ${mode}. Use analyze, rebuild, or format-finder.` });
}
